...
exports.topics = function (request, reply) {
  posts.topics(function (err, topics) {
    if (err) {
      return reply(Boom.wrap(err, 400));
    }
...
exports.addComment = function (request, reply) {
  posts.addComment({
    category: request.payload.category,
    replyTo: request.payload.key,
    ...
    reply.redirect('/thread/' + request.payload.key);
  });
};
...
exports.deleteComment = function (request, reply) {
  posts.deleteComment({
    ...
    reply.redirect('/thread/' + request.payload.replyTo);
  });
};
